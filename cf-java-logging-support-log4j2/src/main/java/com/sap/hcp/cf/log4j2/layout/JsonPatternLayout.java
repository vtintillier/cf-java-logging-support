...
public final class JsonPatternLayout  extends AbstractStringLayout {
    ...
    private MarkerPatternSelector createPatternSelector() {
    	...
    	pMatches[0] = new PatternMatch(Markers.REQUEST_MARKER.getName(), LayoutPatterns.getPattern(PATTERN_KEY.REQUEST));
    	
    	return new MarkerPatternSelector(pMatches, LayoutPatterns.getPattern(PATTERN_KEY.APPLICATION), false, false, this.config);
    }
    
    private PatternSelector createExceptionSelector() {    	
    	return new MarkerPatternSelector(new PatternMatch[0], LayoutPatterns.getPattern(PATTERN_KEY.EXCEPTION),  false, false, this.config);
    }
