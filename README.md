...
Let's say you want to make use of the *servlet filter* feature, then you need to add the following dependency to your POM with property `cf-logging-version` referring to the latest nexus version (currently `3.8.1`):

```xml
<properties>
	<cf-logging-version>3.8.1</cf-logging-version>
</properties>
```
