...
Let's say you want to make use of the *servlet filter* feature, then you need to add the following dependency to your POM with property `cf-logging-version` referring to the latest nexus version (currently `3.4.0`):

```xml
<properties>
	<cf-logging-version>3.4.0</cf-logging-version>
</properties>
```
